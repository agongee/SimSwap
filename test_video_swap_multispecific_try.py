...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        backup_latent_id = None
        count_idx = 0
        count_track = 1

        for target_image_path in target_images_path:
            print(f"{count_idx} : {count_track} : {target_image_path}")
            ...
            if backup_latent_id == None:
                backup_latent_id = latend_id
            else:
                backup_latent_id += latend_id
            count_track -= 1
            if count_track == 0:
                backup_latent_id /= count_list[count_idx]
                count_idx += 1
                count_track = count_list[count_idx]
                target_id_norm_list.append(backup_latent_id.clone())
                backup_latent_id = None
